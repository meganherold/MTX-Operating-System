...
int fork()
{
  ...
  //add copying the opened file descriptors
  for(i = 0; i < NFD; i++)
  {
    if(running->fd[i]) //only copy non-zero entries
    {
      p->fd[i] = running->fd[i];
      p->fd[i]->refcount++;     //there's another process reference now
...
int hop(char *y)
{
  char segment_string[64], *cp;
  int current_segment, new_segment;

  current_segment = running->uss;

  /* get command line line from U space */
   cp = segment_string;
   while( (*cp=get_byte(current_segment, y)) != 0 ){
          y++; cp++;
   }

  printf("hippity hop to %s\n", segment_string);

  printf("segment_string[2] = %c\n", segment_string[2]);
  switch(segment_string[2])
  {
    case '2' : new_segment = 0x2000;  break;
    case '3' : new_segment = 0x3000;  break;
    case '4' : new_segment = 0x4000;  break;
    case '5' : new_segment = 0x5000;  break;
    case '6' : new_segment = 0x6000;  break;
    case '7' : new_segment = 0x7000;  break;
    case '8' : new_segment = 0x8000;  break;
    case '9' : new_segment = 0x9000;  break;

    default:
      printf("that's not a valid segment.\n"); return -1; break;
  }

  printf("new_segment = %x\n", new_segment);

  //copy the image, but don't make a new process
  copyImage(running->uss, new_segment, 32*1024); // copy 32K words
  running->uss = new_segment;

  //*** change uDS, uES, uCS, AX in new ustack ****
  put_word(new_segment, new_segment, running->usp);       // uDS=segment
  put_word(new_segment, new_segment, running->usp+2);     // uES=segment
  put_word(0, new_segment, running->usp+2*8);         // uax=0
  put_word(new_segment, new_segment, running->usp+2*10);  // uCS=segment
